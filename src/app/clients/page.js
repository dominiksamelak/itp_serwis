...
export default function ClientsPage() {
  ...
  const handleClientClick = async (client) => {
    ...
    // Only fetch repairs if we don't have them cached
    if (!repairs[client.id]) {
      const { data, error } = await supabase
      ...
      if (!error) {
        setRepairs((prev) => ({ ...prev, [client.id]: data }));
      }
    }
  };
                  ...
                  <React.Fragment key={client.id}>
                    <tr>
                      <td className={styles.nameCell}>
                        <Link
                          href={`/client-info?clientId=${client.id}`}
                      ...
                      <tr className={styles.expandedRow}>
                        <td colSpan={4} style={{ padding: 0 }}>
                          <div className={styles.repairsSection}>
                            ...
                            {!repairs[client.id] ||
                            repairs[client.id].length === 0 ? (
                              <p className={styles.noRepairs}>
