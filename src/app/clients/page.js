      ...
      <div className={styles.content}>
        <div className={styles.tableHeaderRow}>
          ...
          <div className={styles.pageSizeSelectWrapper}>
            <label htmlFor="clientPageSize" className={styles.pageSizeLabel}>Na stronę:</label>
            <select
              id="clientPageSize"
          ...
          <table className={styles.clientsTable}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr 
                  key={client.id}
                  className={selectedClient?.id === client.id ? styles.selectedRow : ''}
                >
                  ...
                  <td>
                    ...
                    <button
                      ...
                      onClick={() => router.push(`/add-equipment/${client.id}`)}
                    >
                      Dodaj zgłoszenie
                    </button>
                  </td>
        ...
        {getTotalPages(clients) > 1 && (
          <div className={styles.pagination}>
            ...
            {Array.from({ length: getTotalPages(clients) }, (_, i) => i + 1).slice(Math.max(0, page - 5), page + 4).map(p => (
              <button
                key={p}
            ...
            <button disabled={page === getTotalPages(clients)} onClick={() => setPage(page + 1)}>&gt;</button>
            <button disabled={page === getTotalPages(clients)} onClick={() => setPage(getTotalPages(clients))}>&raquo;</button>
          </div>
        )}
      </div>
    </div>
